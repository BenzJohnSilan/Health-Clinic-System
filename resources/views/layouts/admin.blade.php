...
<div class="sidebar" id="sidebar">
    ...
    <div class="sidebar-nav-wrapper">
        ...
        <a href="{{ route('admin.pending') }}"
           class="{{ request()->routeIs('admin.pending') ? 'active' : '' }}">
        ...
        <a href="{{ route('admin.appointments.index') }}"
           class="{{ request()->routeIs('admin.appointments.*') || request()->routeIs('admin.pending-appointments') ? 'active' : '' }}">
        ...
        <a href="{{ route('admin.user-logs') }}"
           class="{{ request()->routeIs('admin.user-logs') ? 'active' : '' }}">
        ...
        <div class="sidebar-user-card">
            @if($admin->avatar && file_exists(storage_path('app/public/' . $admin->avatar)))
                <img class="user-avatar"
                     ...
                     alt="Avatar">
            @else
                <div class="user-avatar-placeholder">{{ $__adminInitials }}</div>
            @endif
            <div class="user-info">
...
(function () {
    ...
    @php
        ...
        // Notifications + unread count are provided by the layouts.admin
        // view composer (see AppServiceProvider::boot) via
        // AdminNotificationService, which cross-references the persistent
        // admin_notification_reads table. $adminNotifications /
        // $adminUnreadCount are always available here, but default them
            ...
            <div class="notif-wrapper">
                ...
                <div class="notif-panel" id="notifPanel">
                    ...
                    <div class="notif-panel-header">
                        ...
                        <span class="notif-panel-count" id="notifPanelCount" style="{{ $adminUnreadCount > 0 ? '' : 'display:none;' }}">
                            {{ $adminUnreadCount }} new
                        </span>
                    </div>

                    <!-- Mark all as read -->
                    <div class="notif-panel-actions" id="notifMarkAllWrap" style="{{ $adminUnreadCount > 0 ? '' : 'display:none;' }}">
                    ...
                    <div class="notif-tabs" id="notifTabs" role="tablist" aria-label="Notification filters">
                        ...
                        <button type="button" class="notif-tab" data-filter="accounts">Accounts</button>
                    ...
                    @if($adminNotifications->isNotEmpty())
                        <div class="notif-panel-footer" style="display:flex; gap:8px;">
                            @if($adminNotifications->where('category', 'accounts')->isNotEmpty())
                                <a href="{{ route('admin.pending') }}" class="notif-view-all" style="flex:1;">
                                    Pending Accounts
                                    <i class='bx bx-right-arrow-alt'></i>
                                </a>
                            @endif
                            @if($adminNotifications->where('category', 'appointments')->isNotEmpty())
                                <a href="{{ route('admin.pending-appointments') }}" class="notif-view-all" style="flex:1;">
            ...
            <div class="header-avatar-group" id="avatarGroup">
                @if($admin->avatar && file_exists(storage_path('app/public/' . $admin->avatar)))
                    <img class="h-avatar"
                         ...
                         alt="Avatar">
                @else
                    <div class="h-avatar-initials">{{ $__adminInitials }}</div>
                @endif

                <span class="h-name">{{ $admin->first_name }}</span>
                ...
                <div class="h-dropdown" id="hDropdown">
                    <div class="h-dropdown-header">
                        ...
                        @else
                            <div class="hd-initials">{{ $__adminInitials }}</div>
                        @endif
                        <div class="hd-info">
                            <span class="hd-name">{{ $admin->first_name }} {{ $admin->last_name }}</span>
                            <span class="hd-role">{{ $admin->role }}</span>
                        </div>
                    </div>
                    <a href="{{ route('admin.settings') }}">
...
(function () {
    ...
    notifList.addEventListener('click', function (e) {
        ...
        fetch(@json(route('admin.notifications.read')), {
            method: 'POST',
            keepalive: true,
            headers: {
    ...
    // Filter tabs (All / Unread / Accounts / Appointments)
    if (tabs) {
