...
<div class="container">
    ...
    <div class="status-tabs">
        <a href="{{ route('admin.appointments.index', array_merge($activeFilters, ['status' => 'all'])) }}"
        ...
        @foreach($statusList as $s)
            <a href="{{ route('admin.appointments.index', array_merge($activeFilters, ['status' => $s])) }}"
    ...
    <form method="GET" action="{{ route('admin.appointments.index') }}" class="filter-bar">
        ...
        <input
            ...
            name="search"
            class="filter-input"
            placeholder="Search patient or reference no..."
        ...
        @if(request('search') || request('doctor_id') || request('date'))
            <a href="{{ route('admin.appointments.index', ['status' => $activeStatus]) }}" class="btn-clear-filter">
                Clear
            </a>
        @endif
    </form>
        ...
        <table class="appointments-table">
            ...
            <tbody>
                ...
                @php
                    ...
                    // (inherited unchanged by AdminAppointmentController).
                ...
                <tr>
                    ...
                    <!-- Actions: View + three-dot menu only -->
                    <td>
    ...
    <form action="{{ route('admin.appointments.approve', $appointment->id) }}" method="POST">
        ...
        <button
            type="submit"
            class="action-menu-item action-menu-approve {{ $appointment->status !== 'Pending' ? 'action-menu-disabled' : '' }}"
    ...
    <button
        type="button"
        class="action-menu-item action-menu-reject openRejectModal {{ $appointment->status !== 'Pending' ? 'action-menu-disabled' : '' }}"
        {{ $appointment->status !== 'Pending' ? 'disabled' : '' }}
    ...
    <form action="{{ route('admin.appointments.cancel', $appointment->id) }}" method="POST">
        ...
        <button
            type="submit"
            class="action-menu-item action-menu-cancel {{ $appointment->status !== 'Approved' ? 'action-menu-disabled' : '' }}"
    ...
    <form action="{{ route('admin.appointments.checkIn', $appointment->id) }}" method="POST">
        ...
        <button
            type="submit"
            class="action-menu-item action-menu-checkin {{ !$isCheckInEligible ? 'action-menu-disabled' : '' }}"
    ...
    <button
        type="button"
        class="action-menu-item action-menu-reschedule openRescheduleModal {{ !$isReschedulable ? 'action-menu-disabled' : '' }}"
        {{ !$isReschedulable ? 'disabled' : '' }}
        data-id="{{ $appointment->id }}"
    ...
    <button
        type="button"
        class="action-menu-item action-menu-no-show openNoShowModal {{ !$isNoShowEligible ? 'action-menu-disabled' : '' }}"
...
<div id="rescheduleModal" class="modal-overlay" style="display:none;">
    <div class="modal-box">
        ...
        <form id="rescheduleForm" method="POST" data-url="{{ route('admin.appointments.reschedule', ':id') }}">
            @csrf
...
<div id="noShowModal" class="modal-overlay" style="display:none;">
    <div class="modal-box">
        ...
        <form id="noShowForm" method="POST" data-url="{{ route('admin.appointments.noShow', ':id') }}">
            @csrf
